...
import {DELIVERED_DELIVERY_LIST_QUERY} from "./queries";
import DeliverySection from "../DeliveryListPage/components/DeliverySection";

class DeliveredDeliveryListPage extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
  ...
  submitHandler = e => {
    ...
    this.props.router.push({
        pathname: '/deliveries/state/delivered',
        query: {
  ...
  loadMore = (fromItem) =>{
    ...
    this.props.data.fetchMore({
      ...
      updateQuery: (previousResult, { fetchMoreResult }) => {
        const addedCampaigns = fetchMoreResult.deliveredCampaigns;
        if (addedCampaigns.length < 1){
          this.setState({hasMore:false})
        }
        return { deliveredCampaigns: [...previousResult.deliveredCampaigns,...addedCampaigns]}
      },
    });
  }

  render() {
    const {data: {error, loading, deliveredCampaigns: campaigns}} = this.props;
    if (loading) return <Loader/>;
    if (error) return <h1>{error.message}</h1>;
    return (
      <MDBContainer className={"px-4 pt-4"}>
        <h1>Delivered Deliveries</h1>
