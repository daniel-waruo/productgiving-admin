...
class DeliveryListPage extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
  ...
  loadMore = (fromItem) =>{
    ...
    this.props.data.fetchMore({
      ...
      updateQuery: (previousResult, { fetchMoreResult }) => {
        const addedCampaigns = fetchMoreResult.closedCampaigns;
        if (addedCampaigns.length < 1){
          this.setState({hasMore:false})
        }
        return { closedCampaigns: [...previousResult.closedCampaigns,...addedCampaigns]}
      },
    });
  }
  render() {
    const {data: {error, loading, closedCampaigns: campaigns}} = this.props;
