...
import {UNAPPROVED_CAMPAIGNS_QUERY} from "./queries";
import Link from "next/link";
import BoolIcon from "../BoolIcon";


class UnapprovedCampaignListPage extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
  ...
  loadMore = (fromItem) =>{
    ...
    this.props.data.fetchMore({
      ...
      updateQuery: (previousResult, { fetchMoreResult }) => {
        const addedCampaigns = fetchMoreResult.unapprovedCampaigns;
        if (addedCampaigns.length < 1){
          this.setState({hasMore:false})
        }
        return { unapprovedCampaigns: [...previousResult.unapprovedCampaigns,...addedCampaigns]}
      },
    });
  }

  render() {
    const {data: {error, loading, unapprovedCampaigns:campaigns}} = this.props;
    if (loading) return <Loader/>
    if (error) return <h1>{error.message}</h1>
    return (
      <MDBContainer className={"px-4 pt-4"}>
        <h1>Unapproved Campaigns</h1>
